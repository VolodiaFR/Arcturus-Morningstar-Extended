...
final class RoomPersistence {
    ...
    void save(State state) throws SQLException {
        try (Connection connection = this.database.openConnection();
             PreparedStatement statement =
                     connection.prepareStatement(SAVE_ROOM_SQL)) {
            ...
            statement.setString(
                    32,
                    databaseBoolean(state.staffPromotedRoom()));
            ...
            statement.setString(
                    42,
                    databaseBoolean(state.buildersClubTrialLocked()));
            statement.setString(
                    43,
                    Objects.requireNonNullElse(
                                    state.buildersClubOriginalState(),
                                    RoomState.OPEN)
                            .name()
                            .toLowerCase());
    ...
    private static String serializeMoodlight(
